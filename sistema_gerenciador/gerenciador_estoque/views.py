...
def desativar_usuario(request, usuario_id):
    try:
        ...
        if sessao:
            ...
            if sessao.is_admin:
                usuario_a_desativar.is_ativo = False
...
def reativar_usuario(request, usuario_id):
    try:
        ...
        if sessao:
            usuario_a_desativar = get_object_or_404(Usuario, id=usuario_id)
            if sessao.is_admin:
                usuario_a_desativar.is_ativo = True
                usuario_a_desativar.save()
...
def reenviar_email_ativacao(request, usuario_id):
    ...
    if usuario:
        if not usuario.is_ativo:
            ...
            messages.info(request, 'Este usuario ja esta ativo')
    return redirect('dashboard')


def enviar_email_ativacao(request, usuario):
    token = gerar_token(usuario.pk)
    current_site = get_current_site(request)
    mail_subject = 'Ative sua conta'
    from_email = 'no-reply@seuestoque.com'
...
def login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                usuario = Usuario.objects.only('id', 'is_ativo').get(email=email, password=hashed_password)
...
def dashboard(request):
    ...
    if usuario_id:
        ...
        if usuario.is_admin:
            ...
            usuarios_ativos = Usuario.objects.filter(is_admin=False, is_ativo=True).count()
